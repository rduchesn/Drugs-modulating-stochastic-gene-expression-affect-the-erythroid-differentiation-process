...
def Sample_Estimate(func,n,args,bounds,nsamples=498,lhs=True,maxeval=int(1e6),output_likelihood=False, output_allruns=False,verbose_success=True,verbose_error=False):
  ...
  output_likelihood (boolean): equivalent to the full_output argument of try_estimate and Estimate (whether or not to return the value of the likelihood together with the optimal parameters)
  output_allruns (boolean): whether or not the results of all initial guess samples should be output or only the one with the optimal likelihood.
  
  Returns:
  --------
  If not output_allruns , the function returnsthe optimal parameter set, in the same format as try_estimate. Otherwise it returns a dictionary with matrices as values instead of vectors."""
  ...
  if output_allruns:
    if output_likelihood:
      return({'error':Error, 'parameters':Parameters})
    else:
      return(Parameters)
  else:
    if output_likelihood:
